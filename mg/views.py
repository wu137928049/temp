...
#mg_path = r'C:\\Users\\wu\\Desktop\\zsxq\\mg.xlsx'
mg_path = '/data/filelist/mg.xlsx'
mg_data = xlrd.open_workbook(mg_path)


def mg(request):
    tables = mg_data.sheets()[0]
    table1 = mg_data.sheets()[1]
    table2 = mg_data.sheets()[2]
    table3 = mg_data.sheets()[3]
    table4 = mg_data.sheets()[4]
    ...
    if one_1[0] == ' ':
        ...
        for i in one_dict:
            #这里的上发和上发费用不是使用的省名称和省代码，所以不需要转换。
            one.append(i[0])
            two.append(i[1])
    ...
    for i in xrange(2, 8):
        #table1和table2是上发和上发费用
        sf = table1.col_values(i)[:41]
        sffy = table2.col_values(i)[:41]
        ...
        if (sf[38] == 0 and sf[39] != 0) or (sf[39] == 0 and sf[38] != 0):
            #这里的代码并不是省代码，所以，这里不需要转换成省名称。
            sf_zero.append(sf_sdm)

        if (sffy[38] == 0 and sffy[39] != 0) or (sffy[39] == 0 and sffy[38] != 0):
            sffy_zero.append(sffy_sdm)

    for i in xrange(2,33):
        xf = table3.col_values(i)[:41]
        xf_sdm = xf[0]
        xf = zh_int(xf[1:])
    ...
    #不转int判断=0有问题，因为直接读出来的是浮点数0.0，而且，这个是带省代码的，如果不截取出来，那么取出来的结果是错误的，实际成了下标37和38
    for i in xrange(2, 4):
        cuod = table4.col_values(i)[:41]
        cuod = zh_int(cuod[1:])
    ...
    if len(sffy_zero) != 0:
        zd_zero['sffy'] = sffy_zero
    else:
        zd_zero['sffy'] = ''

    return render_mako_context(request, 'mg/mg_index.html', {'one': one,
                                                          'two': two,
                                                          'three': three,
                                                          'fore': fore,
                                                          'wu': wu,
...
def xxsj(request,page,sdm):
    '''
    如果page是1,表示数据为上发，如果是2表示为上发费用，如果是3表示为下发，4为错单。
    '''
    page = int(page)
    if page == 3:
        sdm = smc_zh(sdm)

    tables = mg_data.sheets()[page]
    ...
    if page == 2:
        cd = 6
        send_type = '上发费用'
        #由于这里的摘要页和实际的上发费用页的sdm名称是不同的，所以这里一一对应解析。
        #仅仅用于判断，完成后依然是使用摘要页上的名称显示。
        if sdm == 'zonggongsi':
            z_sdm = 'C010FEE'
        if sdm == 'yinyue':
            z_sdm =  'C011FEE'
    ...
    #由于代码中的值传过去的的z_sdm，所以这里需要将sdm也复制给z_sdm
    if page != 2:
        z_sdm = sdm
    ...
    for i in xrange(cd):
        if sdm_list[i] == z_sdm:
            ...
            if page == 3:
                ...
                sname = sdm_zh(sdm)
            if page == 1 or page == 2:
                if sname == 'C010FEE' or sname == 'zonggongsi':
                    sname = '总公司'
                if sname == 'C011FEE' or sname == 'yinyue':
                    sname = '音乐'
                if sname == 'C012FEE' or sname == 'shumei':
                    sname = '数媒'
                if sname == 'C013FEE' or sname == 'shixun':
                    sname = '视讯'
                if sname == 'C014FEE' or sname == 'huyu':
                    sname = '互娱'
                if sname == 'C015FEE' or sname == 'dongman':
                    sname = '动漫'

    return render_mako_context(request, 'mg/xxsj.html',{'info':info,
                                                           'infodr':infodr,
                                                           'infotr':infotr,
                                                           'q_ten_sum':q_ten_sum,
                                                           'q_ten_pj':q_ten_pj,
...
def bo_d(request,name):
    ...
    if name == 'cuod':
        ...
        page = 4
        send_type = '错单'
    #注意范围，这里的范围是7，是因为不包含7，而下边cd = 6是因为长度一共是6，下边在长度的基础上加了2，所以，cd 为8及为2 - 8,
     #2,3,4,5,6,7,为六个。
    if name == 'sf':
        cd = 7
        bd = 7
    ...
    return render_mako_context(request,'mg/bodong.html',{
...
def xq(request):
    ...
    return render_mako_context(request,'mg/xq.html')
...
def quanl(request,sdm,page):
    ...
    if sdm == 'sum':
        ...
        return render_mako_context(request,'mg/quanliang.html',{'sdm':s_mingc,
                                                           'ri_q':ri_q,
                                                           'info':info,
                                                           'leix':leix,
                                                               'page':page,
...
def sfquanl(request,sdm,page):
    ...
    # 由于省代码是6个，
    sdm_list = tables.row_values(0)[2: 8]
    ...
    for i in xrange(6):
        if sdm_list[i] == sdm:
            ...
            info = zh_int(info)
            info = qfw(info)
            if sdm == 'C010FEE' or sdm == 'zonggongsi':
                sdm = '总公司'
            if sdm == 'C011FEE' or sdm == 'yinyue':
                sdm = '音乐'
            if sdm == 'C012FEE' or sdm == 'shumei':
                sdm = '数媒'
            if sdm == 'C013FEE' or sdm == 'shixun':
                sdm = '视讯'
            if sdm == 'C014FEE' or sdm == 'huyu':
                sdm = '互娱'
            if sdm == 'C015FEE' or sdm == 'dongman':
                sdm = '动漫'
            break


    return render_mako_context(request,'mg/quanliang.html',{'sdm':sdm,
                                                           'ri_q':ri_q,
                                                           'info':info,
                                                           'leix':leix,
                                                            'page':page,
...
def cuoql(request,page):
    ...
    return render_mako_context(request,'mg/cuoql.html',{'zd_data':zd_data,
                                                          'leix':leix,
                                                     })

#图形展示
def xftxzs(request,page):
    page = int(page)
    leix = '下发图形展示'
    ...
    return render_mako_context(request, 'mg/xftxzs.html', dictionary={'sdm': sdm,
                                                                                      'd_date': d_date,
                                                                                      'beij': beij,
                                                                                      'bjdr': bjdr,
                                                                                      'bjtr': bjtr,
...
def xfdtxzs(request,page,number):
    ...
    if number == 6:
        ...
        liaon = zh_int(liaon)
        #此页面仅仅用于是七个的页面使用
        return render_mako_context(request, 'mg/xf7txzs.html', {'d_date': d_date,
                                                                 'sdm': sheng_mingc,
                                                                   'ys_sdm':ys_sdm,
                                                                 'beij': beij,
                                                                 'bjdr': bjdr,
    ...
    if number == 7:
        ...
        return render_mako_context(request, 'mg/xfdtxzs.html', {'d_date': d_date,
                                                               'sdm': sheng_mingc,
                                                               'beij': beij_sum,
                                                                   'leix':leix,
                                                                   'number':number,
    ...
    return render_mako_context(request,'mg/xfdtxzs.html',{
                                                              'sdm':sheng_mingc,
                                                            'ys_sdm': ys_sdm,
                                                              'd_date': d_date,
                                                              'beij': beij,
...
def sfsdm_zh(list):
    ...
    for sdm in list:
        if sdm == 'C010FEE' or sdm == 'zonggongsi':
            sdm = '总公司'
            sdm_list.insert(0,sdm)
        if sdm == 'C011FEE' or sdm == 'yinyue':
            sdm = '音乐'
            sdm_list.insert(1, sdm)
        if sdm == 'C012FEE' or sdm == 'shumei':
            sdm = '数媒'
            sdm_list.insert(2, sdm)
        if sdm == 'C013FEE' or sdm == 'shixun':
            sdm = '视讯'
            sdm_list.insert(3, sdm)
        if sdm == 'C014FEE' or sdm == 'huyu':
            sdm = '互娱'
            sdm_list.insert(4, sdm)
        if sdm == 'C015FEE' or sdm == 'dongman':
            sdm = '动漫'
            sdm_list.insert(5, sdm)
    return sdm_list

def sftxzs(request,id_d):
    leix = '上发图形展示'
    id_d = int(id_d)
    if id_d == 2:
        leix = '上发费用图形展示'
    tables = mg_data.sheets()[id_d]
    # 第一行，省代码名称
    #显示出来是汉字，但是传递处理还是用sdms，sdm
    sdms = tables.row_values(0)[1:]
    sdm = sfsdm_zh(sdms)
    sdm.insert(0,'sum')
    ...
    #sum
    s_sum = tables.col_values(1)[1:41]
    s_sum = zh_int(s_sum)

    return render_mako_context(request, 'mg/sftxzs.html', dictionary={'sdm': sdm,
                                                                      'sdms': sdms,
                                                                                    'd_date': d_date,
                                                                                    'beij': beij,
                                                                                    'guangd': guangd,
...
def cuodtxzs(request, id_d):
    ...
    s_sum = tables.col_values(1)[1:41]
    s_sum = zh_int(s_sum)
    return render_mako_context(request, 'mg/cuodtxzs.html', dictionary={'sdm': sdm,
                                                                      'd_date': d_date,
                                                                      'beij': beij,
                                                                      'guangd': guangd,
                                                                      's_sum': s_sum,
